...
set -euo pipefail
BACKUP_DIR=/opt/backups/elite-writer
mkdir -p "$BACKUP_DIR"
STAMP=$(date +%Y%m%d-%H%M)
# DATABASE_URL format: mysql://user:pass@host:port/dbname
URL=$(grep -E '^DATABASE_URL=' /opt/elite-writer-v5/.env | cut -d= -f2-)
...
docker run --rm --network host mysql:8 mysqldump \
  -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASS" --single-transaction --quick "$DB_NAME" \
