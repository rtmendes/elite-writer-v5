...
export default function Writer() {
  ...
  // Plain-text mirror for scoring, word count, AI, export (derived from editorHtml)
  ...
  const handleExport = (format: string) => {
    const mdContent = htmlToMarkdown(editorHtml);
    ...
    <div className="flex h-[calc(100vh-3.5rem)] overflow-hidden">
      ...
      <aside className="w-80 border-l border-border bg-card overflow-y-auto hidden lg:block">
        <Tabs value={sidebarTab} onValueChange={setSidebarTab}>
          <TabsList className="w-full rounded-none border-b border-border bg-transparent h-10 grid grid-cols-7">
            ...
            <TabsTrigger value="dataviz" className="text-[10px] gap-0.5 px-1" title="Data Visualization">
              <BarChart3 className="w-3 h-3" /> <span className="hidden xl:inline">Viz</span>
            </TabsTrigger>
          ...
          {/* Data Visualization Tab (GIVE Engine) */}
